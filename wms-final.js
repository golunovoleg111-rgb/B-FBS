...
(function(){
  ...
  function openImport(){
    if(!requireClientPermission('nomenclature_manage','Импорт номенклатуры для вашей учетной записи отключен.'))return;
  ...
  window.apiModal=function(){
    const admin=currentUser?.role==='admin';
    openModal('Интеграция Wildberries',`<div class="api-status-card"><span class="status-pill" id="wb-api-status">Проверка…</span><h3>Подключение WB API</h3><p>Ключ используется только сервером B-FBS и не возвращается в браузер. Для заказов FBS нужен токен с категорией «Маркетплейс».</p>${admin?`<form class="wms-form" id="wb-token-form"><label>Новый API-токен<input name="token" type="password" autocomplete="new-password" placeholder="Вставьте полный токен WB" required></label><small class="wb-secret-note">После сохранения значение будет скрыто. Новый ключ заменит предыдущий.</small><div class="form-error"></div><div class="modal-actions wb-token-actions"><button type="button" class="btn danger" id="delete-wb-token" hidden>Отключить</button><button class="btn primary">Проверить и сохранить</button></div></form>`:'<p>Добавить или заменить ключ может только администратор.</p>'}<div class="modal-actions"><button class="btn" data-close>Закрыть</button>${['admin','manager'].includes(currentUser?.role)?'<button class="btn" id="go-wb-import">Открыть импорт WB</button>':''}</div></div>`,async(modal,close)=>{
      const status=modal.querySelector('#wb-api-status'),removeButton=modal.querySelector('#delete-wb-token');
      const setStatus=result=>{status.textContent=result.configured?'WB API подключён':'WB API не подключён';status.classList.toggle('ok',result.configured);status.classList.toggle('muted',!result.configured);if(removeButton)removeButton.hidden=!result.configured||result.source==='environment'};
      if(backendAvailable){try{setStatus(await requestApi('/api/wb/status'))}catch(error){status.textContent=error.message;status.classList.add('error')}}else{status.textContent='Сервер недоступен';status.classList.add('error')}
      modal.querySelector('#wb-token-form')?.addEventListener('submit',async event=>{event.preventDefault();const form=event.currentTarget,errorHost=form.querySelector('.form-error'),button=form.querySelector('button[type="submit"]');errorHost.textContent='';button.disabled=true;button.textContent='Проверяем…';try{const result=await requestApi('/api/wb/token',{method:'PUT',body:JSON.stringify({token:form.elements.token.value})});form.reset();setStatus(result);toast(`WB API подключён. Получено новых заказов: ${result.ordersCount}.`,'success')}catch(error){errorHost.innerHTML=`<div class="error">${esc(error.message)}</div>`}finally{button.disabled=false;button.textContent='Проверить и сохранить'}});
      removeButton?.addEventListener('click',async()=>{if(!confirm('Отключить WB API и удалить сохранённый ключ?'))return;removeButton.disabled=true;try{const result=await requestApi('/api/wb/token',{method:'DELETE'});setStatus(result);toast(result.message,'success')}catch(error){toast(error.message,'warning')}finally{removeButton.disabled=false}});
      modal.querySelector('#go-wb-import')?.addEventListener('click',()=>{close();activeTab='inventory';render();setTimeout(openImport,0)});
    });
  };
  ...
  function renderRevisions(){const host=document.getElementById('revisions-list');if(!host)return;host.innerHTML=wms.revisions.length?wms.revisions.map(item=>`<article class="operation-card"><div><span class="priority ${item.priority==='Высокий'?'high':''}">${esc(item.priority)} приоритет</span><h3>${esc(item.boxId)}</h3><p>${esc(warehouseName(item.warehouseId))} · ${esc(findZone(item.warehouseId,item.zoneId)?.name||item.zoneId)}</p><small>${statusLabel(item.status)} · создано ${dateText(item.createdAt)}${item.completedAt?' · проверено '+dateText(item.completedAt):''}</small></div><div class="operation-actions">${can('revisions_manage')&&item.status!=='done'?'<button class="btn primary" data-revision-check="'+esc(item.id)+'">Проверить</button>':''}</div></article>`).join(''):empty('Активных заданий на ревизию нет.');host.querySelectorAll('[data-revision-check]').forEach(button=>button.onclick=()=>openRevision(button.dataset.revisionCheck))}
  ...
  function tasksView(){
    const canCreate=can('tasks_manage'),canPick=can('tasks_pick');
    ...
    return `<section class="wms-page">
      <div class="wms-page-head">
        ...
        <div class="wms-actions">
          ${canPick?`<button class="btn scanner-launch" id="scanner-mode" ${active?'':'disabled'}>Scanner Mode${active?` · ${active}`:''}</button>`:''}
    ...
    host.innerHTML=wms.tasks.length?wms.tasks.map(task=>{
      ...
      return `<article class="operation-card">
        ...
        <div class="operation-actions">
          ${can('tasks_pick')&&!['ready','done','cancelled'].includes(task.status)?'<button class="btn primary" data-task-work="'+esc(task.id)+'">Собирать</button>':''}
          ${can('tasks_manage')&&task.status==='ready'?'<button class="btn primary" data-task-done="'+esc(task.id)+'">Завершить</button>':''}
        </div>
      </article>`;
  ...
  function openBoxForm(box,defaultZoneId){
    if(!requireClientPermission('inventory_manage','У вас нет доступа к изменению коробок и остатков.'))return;
  ...
  function deleteBox(box){
    if(!requireClientPermission('inventory_manage','У вас нет доступа к удалению коробок.'))return false;
    if(!box)return false;
  ...
  function openBulkReplenishment(){
    if(!requireClientPermission('inventory_manage','Массовое пополнение для вашей учетной записи отключено.'))return;
    const warehouseId=inventoryWarehouseId();
