...
(function(){
  ...
  function bindInventory(){
    const search=document.getElementById('inventory-search'),warehouse=document.getElementById('inventory-warehouse');if(search){search.oninput=renderInventoryTable;warehouse.onchange=renderInventoryTable;renderInventoryTable()}
  ...
  function transferView(){return `<section class="wms-page"><div class="wms-page-head"><div><h2>Заявки на перемещение</h2><p>Списание выполняется при отправке, приход — при получении.</p></div>${['admin','manager'].includes(currentUser?.role)?'<button class="btn primary" id="add-transfer">+ Новая заявка</button>':''}</div><div id="transfers-list" class="operation-list"></div></section>`}
  function renderTransfers(){
    const host=document.getElementById('transfers-list');if(!host)return;
    host.innerHTML=wms.transfers.length?wms.transfers.map(transfer=>`<article class="operation-card"><div><span class="status-pill ${transfer.status==='received'?'ok':''}">${esc(statusLabel(transfer.status))}</span><h3>${esc(transfer.article)} · ${esc(transfer.size)}</h3><p>${esc(warehouseName(transfer.fromWarehouseId))} → ${esc(warehouseName(transfer.toWarehouseId))}</p><small>${esc(transfer.barcode)} · ${Number(transfer.quantity)} ед. · ${dateText(transfer.createdAt)}</small></div><div class="operation-actions">${transfer.status==='new'?'<button class="btn" data-transfer-send="'+esc(transfer.id)+'">Отправить</button>':''}${transfer.status==='in_transit'?'<button class="btn primary" data-transfer-receive="'+esc(transfer.id)+'">Принять</button>':''}${!['received','cancelled'].includes(transfer.status)?'<button class="btn danger-outline" data-transfer-cancel="'+esc(transfer.id)+'">Отменить</button>':''}</div></article>`).join(''):empty('Заявок на перемещение пока нет.');
  ...
  function bindTransfers(){if(document.getElementById('transfers-list'))renderTransfers();document.getElementById('add-transfer')?.addEventListener('click',openTransferForm)}
  function openTransferForm(){
    if(!wms.boxes.some(box=>boxQuantity(box)>0)){toast('Нет коробок с остатками для перемещения.','warning');return}
    const source=wms.boxes.find(box=>boxQuantity(box)>0),target=warehouses().find(item=>item.id!==source.warehouseId);
    if(!target){toast('Для перемещения создайте второй склад.','warning');return}
    openModal('Новая заявка',`<form class="wms-form" id="transfer-form"><label>Коробка-источник<select name="boxId">${wms.boxes.filter(box=>boxQuantity(box)>0).map(box=>`<option value="${esc(box.id)}">${esc(box.id)} · ${esc(warehouseName(box.warehouseId))} · ${boxQuantity(box)} ед.</option>`).join('')}</select></label><label>Товар<select name="barcode" id="transfer-sku"></select></label><label>Склад назначения<select name="toWarehouseId">${warehouseOptions(target.id,source.warehouseId)}</select></label><label>Количество<input name="quantity" type="number" min="1" value="1" required></label><div class="form-error"></div><div class="modal-actions"><button type="button" class="btn" data-close>Отмена</button><button class="btn primary">Создать</button></div></form>`,(modal,close)=>{
      ...
      const update=()=>{const box=wms.boxes.find(item=>item.id===boxSelect.value);skuSelect.innerHTML=(box?.items||[]).filter(item=>Number(item.quantity)>0).map(item=>`<option value="${esc(item.barcode)}">${esc(item.article)} · ${esc(item.size)} · ${Number(item.quantity)} ед.</option>`).join('');targetSelect.innerHTML=warehouseOptions('',box?.warehouseId)};boxSelect.onchange=update;update();
      form.onsubmit=event=>{event.preventDefault();const data=Object.fromEntries(new FormData(form)),box=wms.boxes.find(item=>item.id===data.boxId),item=box?.items.find(row=>row.barcode===data.barcode),quantity=Number(data.quantity);if(!item||quantity<1||quantity>Number(item.quantity)){form.querySelector('.form-error').innerHTML='<div class="error">Количество превышает остаток в коробке.</div>';return}const targetZones=warehouses().find(warehouse=>warehouse.id===data.toWarehouseId)?.zones||[];if(!targetZones.length){form.querySelector('.form-error').innerHTML='<div class="error">На складе назначения нет зон хранения.</div>';return}wms.transfers.unshift({id:uid('TR'),status:'new',boxId:box.id,fromWarehouseId:box.warehouseId,toWarehouseId:data.toWarehouseId,toZoneId:targetZones[0].id,article:item.article,barcode:item.barcode,size:item.size,quantity,createdAt:new Date().toISOString(),createdBy:currentUser?.name||''});saveWms(`Создана заявка на перемещение ${item.article}`);close();renderTransfers()};
    });
  }
  function sendTransfer(id){const transfer=wms.transfers.find(item=>item.id===id),box=wms.boxes.find(item=>item.id===transfer?.boxId),stock=box?.items.find(item=>item.barcode===transfer?.barcode);if(!transfer||transfer.status!=='new'||!stock)return;if(Number(stock.quantity)<transfer.quantity){toast('Остатка в исходной коробке уже недостаточно.','warning');return}stock.quantity-=transfer.quantity;box.updatedAt=new Date().toISOString();transfer.status='in_transit';transfer.sentAt=new Date().toISOString();recordMovement('transfer_out',-transfer.quantity,box,stock,transfer.id);saveWms(`Отправлено перемещение ${transfer.id}`);renderTransfers()}
  ...
  function openRevision(id){const revision=wms.revisions.find(item=>item.id===id),box=wms.boxes.find(item=>item.id===revision?.boxId);if(!revision||!box)return;openModal(`Ревизия ${box.id}`,`<form class="wms-form" id="revision-form"><p>Укажите фактическое количество по каждой позиции.</p>${(box.items||[]).map((item,index)=>`<label>${esc(item.article)} · ${esc(item.size)} · ${esc(item.barcode)}<input name="qty-${index}" type="number" min="0" value="${Number(item.quantity||0)}"></label>`).join('')||empty('Коробка пуста')}<label>Комментарий<textarea name="note" rows="3"></textarea></label><div class="modal-actions"><button type="button" class="btn" data-close>Отмена</button><button class="btn primary">Завершить проверку</button></div></form>`,(modal,close)=>{modal.querySelector('form').onsubmit=event=>{event.preventDefault();const form=event.currentTarget;(box.items||[]).forEach((item,index)=>{const actual=Math.max(0,Number(form.elements[`qty-${index}`].value)||0),difference=actual-Number(item.quantity||0);if(difference)recordMovement('revision_adjustment',difference,box,item,form.elements.note.value);item.quantity=actual});box.updatedAt=new Date().toISOString();revision.status='done';revision.completedAt=new Date().toISOString();revision.completedBy=currentUser?.name||'';revision.note=form.elements.note.value;saveWms(`Завершена ревизия ${box.id}`);close();renderRevisions()}})}
